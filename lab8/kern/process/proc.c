...
copy_thread(struct proc_struct *proc, uintptr_t esp, struct trapframe *tf) {
    ...
    *(proc->tf) = *tf;
    ...
    proc->tf->tf_regs.reg_r[MIPS_REG_SP] = esp ? esp : (uintptr_t)proc->tf - 32;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    if (copy_mm(clone_flags, proc)) goto bad_fork_cleanup_files;
...
load_icode(int fd, int argc, char **kargv) {
	...
	struct elfhdr32 elfdata, *elf = &elfdata;
	if ((ret = load_icode_read(fd, &___elfhdr__, sizeof(struct __elfhdr), 0)) != 0) {
        goto bad_elf_cleanup_pgdir;
    }

    _load_elfhdr((unsigned char*)&___elfhdr__, elf);
	...
	struct proghdr phdata;

    struct proghdr *ph = &phdata, *phptr1 = elf->e_phoff, *phs = phptr1 + elf->e_phnum;

    uint32_t vm_flags, perm;
    for (; phptr1 < phs; phptr1++) {
	load_icode_read(fd, ph, sizeof(struct proghdr), phptr1);
